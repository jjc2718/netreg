...
p.add_argument('--num_features', type=int, default=-1)
...
if args.num_features > 0:
    if args.random:
        # subset features randomly
        feats_to_use = np.random.choice(rnaseq_train_df.columns.values,
                                        size=args.num_features)
        rnaseq_train_df = rnaseq_train_df.loc[:, feats_to_use]
        rnaseq_test_df = rnaseq_test_df.loc[:, feats_to_use]
    else:
        ...
        coefs_df['feature'] = coefs_df['feature'].astype('int').astype('str')
        coefs_df = coefs_df.drop(columns='abs')
        coefs_df['abs'] = coefs_df['weight'].abs()
        feats_to_use = coefs_df.sort_values(by='abs', ascending=False)['feature'].values[:args.num_features]
        rnaseq_train_df = rnaseq_train_df.loc[:, feats_to_use]
        rnaseq_test_df = rnaseq_test_df.loc[:, feats_to_use]
    ...
    for signal in ["signal"]:
        ...
        if hasattr(torch_model, 'results_df'):
            torch_model.results_df.to_csv(os.path.join(args.results_dir,
                                                       'torch_params_{}_{}_n{}.tsv'.format(
                                                       'random' if args.random else 'top',
                                                       args.seed, args.num_features)), sep='\t')
        else:
